#!/usr/bin/env python3
# To enable auto switching, create a symlink to this script in the ~/Library/ApplicationSupport/iTerm2/Scripts/AutoLaunch folder

import asyncio
import iterm2

# Note, these color presets must be imported via profiles->[profile
# name]->colors->color presets...
THEME_LIGHT = "Tango Light"
THEME_DARK = "brycedcarter"
...
class AutoSwtichTheme:
    ...
    async def set_color_preset(self, theme):
        preset = await iterm2.ColorPreset.async_get(
            self.connection, self.light if theme == "light minimal" else self.dark
        )
